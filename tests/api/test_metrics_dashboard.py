...
from api.metrics_dashboard import MetricsDashboard
...
class TestMetricsDashboard:
    ...
    async def test_snapshot_action_returns_collector_data(self):
        ...
        with patch("api.metrics_dashboard.collector.snapshot", return_value=mock_snapshot):
    ...
    async def test_default_action_is_snapshot(self):
        handler = _make_handler()
        with patch("api.metrics_dashboard.collector.snapshot", return_value={}):
    ...
    async def test_clear_action_clears_and_returns_message(self):
        handler = _make_handler()
        with patch("api.metrics_dashboard.collector.clear"):
