...
from api.settings_get import GetSettings
...
class TestGetSettings:
    ...
    async def test_returns_settings_dict(self):
        ...
        with patch("api.settings_get.settings.get_settings", return_value=mock_backend), \
             patch("api.settings_get.settings.convert_out", return_value=mock_out):
    ...
    async def test_calls_get_settings(self):
        handler = _make_handler()
        with patch("api.settings_get.settings.get_settings") as mock_get, \
             patch("api.settings_get.settings.convert_out", return_value={}):
    ...
    async def test_calls_convert_out_with_backend(self):
        ...
        with patch("api.settings_get.settings.get_settings", return_value=mock_backend), \
             patch("api.settings_get.settings.convert_out") as mock_convert:
            await handler.process({}, MagicMock())
