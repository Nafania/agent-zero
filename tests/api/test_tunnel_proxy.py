...
from api.tunnel_proxy import TunnelProxy, process
...
class TestTunnelProxyProcess:
    ...
    async def test_forwards_to_local_process_when_service_fails(self):
        with patch("api.tunnel_proxy.requests.post", side_effect=Exception("Connection refused")), \
             patch("api.tunnel.process", new_callable=AsyncMock, return_value={"success": True}):
    ...
    async def test_forwards_to_tunnel_service_when_ok(self):
        ...
        with patch("api.tunnel_proxy.requests.post", side_effect=[health_resp, data_resp]):
    ...
    async def test_returns_error_on_json_decode_failure(self):
        ...
        with patch("api.tunnel_proxy.requests.post", return_value=mock_response):
            result = await process({"action": "get"})
...
class TestTunnelProxyHandler:
    ...
    async def test_handler_delegates_to_process(self):
        ...
        with patch("api.tunnel_proxy.process", new_callable=AsyncMock, return_value={"success": True}):
