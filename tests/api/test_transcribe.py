...
class TestTranscribe:
    ...
    async def test_transcribe_returns_result_on_success(self):
        ...
        with patch("api.transcribe.settings.get_settings", return_value={"stt_model_size": "base"}), \
             patch("api.transcribe.whisper.transcribe", new_callable=AsyncMock, return_value=mock_result):
    ...
    async def test_transcribe_use_context_when_ctxid_provided(self):
        ...
        with patch.object(handler, "use_context", return_value=mock_ctx) as mock_use_ctx, \
             patch("api.transcribe.settings.get_settings", return_value={"stt_model_size": "base"}), \
             patch("api.transcribe.whisper.transcribe", new_callable=AsyncMock, return_value={}):
    ...
    async def test_transcribe_passes_settings_to_whisper(self):
        handler = _make_handler()
        with patch("api.transcribe.settings.get_settings", return_value={"stt_model_size": "large-v3"}) as mock_settings, \
             patch("api.transcribe.whisper.transcribe", new_callable=AsyncMock) as mock_transcribe:
