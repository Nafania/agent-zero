...
from api.synthesize import Synthesize
...
class TestSynthesize:
    ...
    async def test_synthesize_returns_audio_on_success(self):
        ...
        with patch("api.synthesize.kokoro_tts.synthesize_sentences", new_callable=AsyncMock, return_value=mock_audio):
    ...
    async def test_synthesize_uses_empty_text_default(self):
        handler = _make_handler()
        with patch("api.synthesize.kokoro_tts.synthesize_sentences", new_callable=AsyncMock) as mock_synth:
    ...
    async def test_synthesize_use_context_when_ctxid_provided(self):
        ...
        with patch.object(handler, "use_context", return_value=mock_ctx) as mock_use_ctx, \
             patch("api.synthesize.kokoro_tts.synthesize_sentences", new_callable=AsyncMock, return_value=b"audio"):
    ...
    async def test_synthesize_returns_error_on_exception(self):
        handler = _make_handler()
        with patch("api.synthesize.kokoro_tts.synthesize_sentences", new_callable=AsyncMock, side_effect=Exception("TTS failed")):
