...
class TestPoll:
    ...
    async def test_process_calls_build_snapshot(self):
        ...
        with patch("api.poll.build_snapshot", new_callable=AsyncMock, return_value=mock_snapshot):
    ...
    async def test_process_passes_context_to_build_snapshot(self):
        handler = _make_handler()
        with patch("api.poll.build_snapshot", new_callable=AsyncMock) as mock_build:
