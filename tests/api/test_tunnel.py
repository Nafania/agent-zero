"""Tests for api/tunnel.py — Tunnel API handler."""
...
from api.tunnel import Tunnel, process, stop
...
class TestTunnelProcess:
    ...
    async def test_get_action_returns_tunnel_status(self):
        ...
        with patch("api.tunnel.TunnelManager.get_instance", return_value=mock_manager):
    ...
    async def test_create_action_starts_tunnel(self):
        ...
        with patch("api.tunnel.TunnelManager.get_instance", return_value=mock_manager), \
             patch("api.tunnel.runtime.get_web_ui_port", return_value=5000):
...
class TestTunnelHandler:
    ...
    async def test_handler_delegates_to_process(self):
        ...
        with patch("api.tunnel.process", new_callable=AsyncMock, return_value={"success": True}):
