"""Tests for api/mcp_servers_apply.py — McpServersApply API handler."""
...
from api.mcp_servers_apply import McpServersApply
...
class TestMcpServersApply:
    ...
    async def test_process_returns_success_with_status(self, mock_app):
        ...
        with patch(
            "api.mcp_servers_apply.set_settings_delta",
        ), patch(
            "api.mcp_servers_apply.asyncio.sleep",
            new_callable=AsyncMock,
        ), patch(
            "api.mcp_servers_apply.MCPConfig"
        ) as MockMCP:
