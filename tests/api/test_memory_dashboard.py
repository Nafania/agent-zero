...
def _reset_state():
    ...
    from plugins._memory.api.memory_dashboard import _dashboard_cache
...
class TestGetCognifyStatus:
    ...
    async def test_returns_worker_status(self):
        ...
        with patch(
            "plugins._memory.helpers.cognee_background.CogneeBackgroundWorker"
        ) as MockBg:
...
class TestGetMemorySubdirs:
    ...
    async def test_returns_subdirs(self):
        dashboard = _make_dashboard()
        with patch(
            "plugins._memory.api.memory_dashboard.get_existing_memory_subdirs",
...
class TestGetCurrentMemorySubdir:
    ...
    async def test_returns_default_when_context_not_found(self):
        dashboard = _make_dashboard()
        with patch("plugins._memory.api.memory_dashboard.AgentContext") as MockCtx:
...
class TestSearchMemoriesOffset:
    ...
    async def test_offset_pagination(self):
        ...
        with patch.dict("sys.modules", {"cognee": mock_cognee}), \
             patch("plugins._memory.api.memory_dashboard.Memory") as MockMem:
...
class TestDashboardCallsSetup:
    ...
    async def test_process_calls_get_cognee_before_action(self):
        ...
        dashboard = _make_dashboard()
        with patch("plugins._memory.api.memory_dashboard.get_existing_memory_subdirs", return_value=["default"]):
...
class TestDashboardCaching:
    ...
    async def test_cache_expires_after_ttl(self, tmp_path):
        ...
        from plugins._memory.api.memory_dashboard import _dashboard_cache, _CACHE_TTL
        dashboard = _make_dashboard()
        req_input = {"action": "search", "memory_subdir": "default", "search": ""}
