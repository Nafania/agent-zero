...
class TestSkillsCatalogExtension:
    ...
    async def test_injects_catalog_when_skills_exist(self, mock_agent):
        from plugins._skills.helpers.skills import Skill
        mock_skills = [
            Skill(
                name="brainstorming",
        ...
        with patch("plugins._skills.extensions.python.message_loop_prompts_after._60_skills_catalog.skills.list_skills", return_value=mock_skills):
            from plugins._skills.extensions.python.message_loop_prompts_after._60_skills_catalog import SkillsCatalogPrompt
    ...
    async def test_skips_when_no_skills(self, mock_agent):
        with patch("plugins._skills.extensions.python.message_loop_prompts_after._60_skills_catalog.skills.list_skills", return_value=[]):
            from plugins._skills.extensions.python.message_loop_prompts_after._60_skills_catalog import SkillsCatalogPrompt
            ext = SkillsCatalogPrompt.__new__(SkillsCatalogPrompt)
            ext.agent = mock_agent
            loop_data = MagicMock()
