...
def test_configure_cognee_alone_makes_get_cognee_work():
    ...
    with patch("plugins._memory.helpers.cognee_init.dotenv") as mock_dotenv, \
         patch("plugins._memory.helpers.cognee_init.get_settings", return_value={
...
async def test_init_cognee_succeeds_on_retry_after_failure():
    ...
    import plugins._memory.helpers.cognee_init as ci
    from plugins._memory.helpers.memory import reload
    ...
    with patch("plugins._memory.helpers.cognee_init.dotenv") as mock_dotenv, \
        ...
        with patch("plugins._memory.helpers.cognee_init.get_settings", return_value={
            "util_model_provider": "openai",
            "util_model_name": "gpt-4o-mini",
            "util_model_api_base": "",
            "embed_model_provider": "huggingface",
...
async def test_memory_dashboard_handles_db_error_gracefully():
    ...
    with patch("api.memory_dashboard.Memory") as MockMem, \
         patch.dict("sys.modules", {"cognee": mock_cognee}):
...
async def test_init_cognee_propagates_configure_failure():
    ...
    from plugins._memory.helpers.cognee_init import init_cognee

    with patch(
        "plugins._memory.helpers.cognee_init.configure_cognee",
