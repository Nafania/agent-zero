...
class TestSearchEngineExecute:
    ...
    async def test_returns_formatted_results(self, tool):
        ...
        with patch("tools.search_engine.searxng", new_callable=AsyncMock, return_value=mock_results):
    ...
    async def test_search_exception_propagates(self, tool):
        with patch("tools.search_engine.searxng", new_callable=AsyncMock, side_effect=Exception("Search failed")):
