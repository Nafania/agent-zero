...
@pytest.fixture
def tool(mock_agent):
    from plugins._memory.tools.memory_load import MemoryLoad
    return MemoryLoad(
        agent=mock_agent,
        name="memory_load",
        method=None,
...
class TestMemoryLoadExecute:
    ...
    async def test_no_results_returns_not_found_message(self, tool):
        ...
        with patch("plugins._memory.tools.memory_load.Memory.get", new_callable=AsyncMock, return_value=mock_db):
    ...
    async def test_results_returned_as_text(self, tool):
        ...
        with patch("plugins._memory.tools.memory_load.Memory.get", new_callable=AsyncMock, return_value=mock_db):
            with patch("plugins._memory.tools.memory_load.Memory.format_docs_plain", return_value=["Doc 1", "Doc 2"]):
