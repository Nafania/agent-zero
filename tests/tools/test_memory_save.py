...
@pytest.fixture
def tool(mock_agent):
    from plugins.memory.tools.memory_save import MemorySave
    return MemorySave(
        agent=mock_agent,
        name="memory_save",
        method=None,
...
class TestMemorySaveExecute:
    ...
    async def test_save_returns_response_with_id(self, tool):
        ...
        with patch("plugins.memory.tools.memory_save.Memory.get", new_callable=AsyncMock, return_value=mock_db):
