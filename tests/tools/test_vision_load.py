"""Tests for tools/vision_load.py — VisionLoad tool."""
...
@pytest.fixture
def tool(mock_agent):
    from plugins.vision.tools.vision_load import VisionLoad
    t = VisionLoad(
        agent=mock_agent,
        name="vision_load",
        method=None,
...
class TestVisionLoadExecute:
    ...
    async def test_skips_nonexistent_paths(self, tool):
        with patch("plugins.vision.tools.vision_load.runtime.call_development_function", new_callable=AsyncMock) as mock_exists:
    ...
    async def test_processes_existing_image_path(self, tool):
        ...
        with patch("plugins.vision.tools.vision_load.runtime.call_development_function", new_callable=AsyncMock, side_effect=mock_dev):
            with patch("plugins.vision.tools.vision_load.images.compress_image", return_value=b"\xff\xd8\xff"):
    ...
    async def test_handles_image_processing_error(self, tool):
        with patch("plugins.vision.tools.vision_load.runtime.call_development_function", new_callable=AsyncMock) as mock_dev:
            async def mock_dev_impl(fn, *args):
                if fn.__name__ == "exists":
                    return True
                raise ValueError("Invalid image")
            mock_dev.side_effect = mock_dev_impl
            with patch("plugins.vision.tools.vision_load.PrintStyle") as mock_ps:
