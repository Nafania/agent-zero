...
try:
    from plugins._scheduler.tools.scheduler import SchedulerTool
except (ImportError, AttributeError):
    SchedulerTool = None

pytestmark = pytest.mark.skipif(SchedulerTool is None, reason="Cannot import SchedulerTool")
...
class TestSchedulerToolListTasks:
    ...
    async def test_list_tasks_returns_json(self, tool, mock_scheduler):
        ...
        with patch("plugins._scheduler.tools.scheduler.TaskScheduler.get", return_value=mock_scheduler):
            resp = await tool.execute()
        assert resp.break_loop is False
...
class TestSchedulerToolCreateScheduledTask:
    ...
    async def test_create_scheduled_task_invalid_cron(self, tool, mock_scheduler):
        ...
        with patch("plugins._scheduler.tools.scheduler.TaskScheduler.get", return_value=mock_scheduler):
            with patch("plugins._scheduler.tools.scheduler.get_context_project_name", return_value=None):
                with patch("plugins._scheduler.tools.scheduler.load_basic_project_data", side_effect=Exception):
                    resp = await tool.execute(
                        name="Test",
                        system_prompt="",
                        prompt="",
...
class TestSchedulerToolCreateAdhocTask:
    ...
    async def test_create_adhoc_task_success(self, tool, mock_scheduler):
        ...
        with patch("plugins._scheduler.tools.scheduler.TaskScheduler.get", return_value=mock_scheduler):
            with patch("plugins._scheduler.tools.scheduler.get_context_project_name", return_value=None):
                with patch("plugins._scheduler.tools.scheduler.load_basic_project_data", return_value={}):
                    with patch("plugins._scheduler.tools.scheduler.AdHocTask") as MockTask:
                        mock_task = MagicMock()
...
class TestSchedulerToolCreatePlannedTask:
    ...
    async def test_create_planned_task_invalid_datetime(self, tool, mock_scheduler):
        ...
        with patch("plugins._scheduler.tools.scheduler.TaskScheduler.get", return_value=mock_scheduler):
            with patch("plugins._scheduler.tools.scheduler.get_context_project_name", return_value=None):
                with patch("plugins._scheduler.tools.scheduler.load_basic_project_data", return_value={}):
                    with patch("plugins._scheduler.tools.scheduler.parse_datetime", return_value=None):
                        resp = await tool.execute(
                            name="Planned",
                            system_prompt="",
                            prompt="",
