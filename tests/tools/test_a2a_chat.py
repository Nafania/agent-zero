...
@pytest.fixture
def tool(mock_agent):
    from plugins._a2a.tools.a2a_chat import A2AChatTool
    return A2AChatTool(
        agent=mock_agent,
        name="a2a_chat",
        method=None,
...
class TestA2AChatToolExecute:
    ...
    async def test_client_not_available_returns_message(self, tool):
        with patch("plugins._a2a.tools.a2a_chat.is_client_available", return_value=False):
    ...
    async def test_missing_agent_url_returns_error(self, tool):
        with patch("plugins._a2a.tools.a2a_chat.is_client_available", return_value=True):
    ...
    async def test_success_returns_assistant_text(self, tool):
        ...
        with patch("plugins._a2a.tools.a2a_chat.is_client_available", return_value=True):
            with patch("plugins._a2a.tools.a2a_chat.connect_to_agent", new_callable=AsyncMock) as mock_connect:
    ...
    async def test_exception_returns_error_message(self, tool):
        with patch("plugins._a2a.tools.a2a_chat.is_client_available", return_value=True):
            with patch("plugins._a2a.tools.a2a_chat.connect_to_agent", new_callable=AsyncMock, side_effect=Exception("Connection failed")):
                resp = await tool.execute(agent_url="http://agent", message="hi")
