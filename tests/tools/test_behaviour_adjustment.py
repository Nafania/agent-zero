...
@pytest.fixture
def tool(mock_agent):
    from tools.behaviour_adjustment import UpdateBehaviour
    t = UpdateBehaviour(
        agent=mock_agent,
...
class TestUpdateBehaviourExecute:
    ...
    async def test_execute_returns_updated_message(self, tool):
        with patch("tools.behaviour_adjustment.update_behaviour", new_callable=AsyncMock):
...
class TestUpdateBehaviourHelpers:
    def test_get_custom_rules_file(self, mock_agent):
        from tools.behaviour_adjustment import get_custom_rules_file
        with patch("tools.behaviour_adjustment.files.get_abs_path", return_value="/abs/behaviour.md"):
            with patch("tools.behaviour_adjustment.memory.get_memory_subdir_abs", return_value="/mem"):
    ...
    def test_read_rules_uses_default_when_no_file(self, mock_agent):
        from tools.behaviour_adjustment import read_rules
        with patch("tools.behaviour_adjustment.get_custom_rules_file", return_value="/nonexistent/behaviour.md"):
            with patch("tools.behaviour_adjustment.files.exists", return_value=False):
    ...
    def test_read_rules_uses_existing_file(self, mock_agent):
        from tools.behaviour_adjustment import read_rules
        with patch("tools.behaviour_adjustment.get_custom_rules_file", return_value="/exists/behaviour.md"):
            with patch("tools.behaviour_adjustment.files.exists", return_value=True):
...
class TestUpdateBehaviourUpdateBehaviour:
    ...
    async def test_update_behaviour_calls_utility_model_and_writes_file(self, mock_agent, mock_log):
        from tools.behaviour_adjustment import update_behaviour
        mock_agent.call_utility_model = AsyncMock(return_value="Merged rules content")
        with patch("tools.behaviour_adjustment.read_rules", return_value="current rules"):
            with patch("tools.behaviour_adjustment.get_custom_rules_file", return_value="/tmp/behaviour.md"):
                with patch("tools.behaviour_adjustment.files.write_file", MagicMock()) as mock_write:
