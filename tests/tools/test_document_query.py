...
@pytest.fixture
def tool(mock_agent):
    from plugins.document_query.tools.document_query import DocumentQueryTool
    t = DocumentQueryTool(
        agent=mock_agent,
        name="document_query",
        method=None,
...
class TestDocumentQueryToolExecute:
    ...
    async def test_single_document_no_queries_returns_content(self, tool):
        with patch("plugins.document_query.tools.document_query.DocumentQueryHelper") as MockHelper:
