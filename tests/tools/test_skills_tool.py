...
@pytest.fixture
def tool(mock_agent):
    from plugins.skills.tools.skills_tool import SkillsTool
    return SkillsTool(
        agent=mock_agent,
        name="skills",
        method="list",
...
class TestSkillsToolList:
    ...
    async def test_list_returns_no_skills_when_empty(self, tool):
        with patch("plugins.skills.tools.skills_tool.skills_helper.list_skills", return_value=[]):
    ...
    async def test_list_returns_skills_when_present(self, tool):
        ...
        with patch("plugins.skills.tools.skills_tool.skills_helper.list_skills", return_value=[mock_skill]):
            resp = await tool.execute(method="list")
...
class TestSkillsToolLoad:
    ...
    async def test_load_skill_not_found(self, tool):
        with patch("plugins.skills.tools.skills_tool.skills_helper.find_skill", return_value=None):
    ...
    async def test_load_success(self, tool):
        ...
        with patch("plugins.skills.tools.skills_tool.skills_helper.find_skill", return_value=mock_skill):
