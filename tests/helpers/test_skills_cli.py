...
def mock_subprocess():
    ...
    async def _transparent_wait_for(coro, *, timeout=None):
        return await coro

    with patch("plugins._skills.helpers.skills_cli.asyncio.create_subprocess_exec") as mock_exec, \
         patch("plugins._skills.helpers.skills_cli.asyncio.wait_for", new=_transparent_wait_for):
        process = AsyncMock()
...
class TestRunNpx:
    ...
    async def test_runs_npx_with_args(self, mock_subprocess):
        from plugins._skills.helpers.skills_cli import _run_npx
    ...
    async def test_raises_on_nonzero_exit(self, mock_subprocess):
        from plugins._skills.helpers.skills_cli import _run_npx, SkillsCLIError
...
class TestParseFindOutput:
    def test_parses_skill_entries(self):
        from plugins._skills.helpers.skills_cli import parse_find_output
        output = (
...
class TestParseListOutput:
    def test_parses_skill_names_and_descriptions(self):
        from plugins._skills.helpers.skills_cli import parse_list_output
        output = (
            "│  Available Skills\n"
            "│\n"
            "│    brainstorming\n"
...
class TestFind:
    ...
    async def test_find_returns_parsed_results(self, mock_subprocess):
        from plugins._skills.helpers.skills_cli import find
        _, process = mock_subprocess
    ...
    async def test_find_uses_cache(self, mock_subprocess):
        from plugins._skills.helpers.skills_cli import find, _cache
        _cache.clear()
        _, process = mock_subprocess
        process.communicate.return_value = (
...
class TestAdd:
    ...
    async def test_add_single_skill(self, mock_subprocess):
        from plugins._skills.helpers.skills_cli import add, _cache
        _cache.clear()
        _, process = mock_subprocess
    ...
    async def test_add_multi_skill_repo(self, mock_subprocess):
        ...
        with patch("plugins._skills.helpers.skills_cli.list_repo_skills",
...
class TestRemove:
    ...
    async def test_remove_calls_npx_skills_remove(self, mock_subprocess):
        from plugins._skills.helpers.skills_cli import remove, _cache
        _cache.clear()
        _, process = mock_subprocess
...
class TestCheckUpdates:
    ...
    async def test_check_calls_npx_skills_check(self, mock_subprocess):
        from plugins._skills.helpers.skills_cli import check_updates
        _, process = mock_subprocess
...
class TestUpdate:
    ...
    async def test_update_calls_npx_skills_update(self, mock_subprocess):
        from plugins._skills.helpers.skills_cli import update
        _, process = mock_subprocess
