...
from plugins.memory.helpers.memory import stable_memory_id_fallback
...
class TestSubmitMemoryFeedback:
    ...
    async def test_queue_only_when_callable_missing(self, feedback_payload, tmp_path, caplog):
        ...
        with patch("plugins.memory.helpers.cognee_feedback.files.get_abs_path", side_effect=_abs_path_mock(tmp_path)), \
             patch("plugins.memory.helpers.cognee_feedback.get_settings", return_value={"cognee_feedback_enabled": True}):
    ...
    async def test_queued_when_feedback_disabled(self, feedback_payload, tmp_path):
        ...
        cognee.session.add_feedback = AsyncMock(return_value=True)
        with patch("plugins.memory.helpers.cognee_feedback.files.get_abs_path", side_effect=_abs_path_mock(tmp_path)), \
             patch("plugins.memory.helpers.cognee_feedback.get_settings", return_value={"cognee_feedback_enabled": False}):
            result = await cf.submit_memory_feedback(feedback_payload, cognee_module=cognee)
        assert result["status"] == "queued"
    ...
    async def test_failed_when_enqueue_raises(self, feedback_payload, tmp_path):
        ...
        with patch("plugins.memory.helpers.cognee_feedback.drain_feedback_queue", new=AsyncMock()), \
             patch("plugins.memory.helpers.cognee_feedback.files.get_abs_path", side_effect=bad_abs), \
             patch("plugins.memory.helpers.cognee_feedback.get_settings", return_value={"cognee_feedback_enabled": True}):
            result = await cf.submit_memory_feedback(feedback_payload, cognee_module=cognee)
