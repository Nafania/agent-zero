...
class TestLocalInteractiveSession:
    ...
    async def test_connect_creates_session_and_starts(self):
        ...
        with patch("plugins.code_execution.helpers.shell_local.tty_session") as mock_ttm:
            mock_ttm.TTYSession.return_value = mock_session
            with patch("plugins.code_execution.helpers.shell_local.runtime") as mock_runtime:
    ...
    async def test_read_output_returns_cleaned_output(self):
        ...
        with patch("plugins.code_execution.helpers.shell_local.clean_string", side_effect=lambda x: x.strip()):
            from plugins.code_execution.helpers.shell_local import LocalInteractiveSession
