...
class TestRunLoop:
    async def test_run_loop_calls_scheduler_tick_when_keep_running(self):
        from plugins._scheduler.helpers import job_loop
...
class TestSchedulerTick:
    async def test_scheduler_tick_calls_task_scheduler_tick(self):
        from plugins._scheduler.helpers.job_loop import scheduler_tick

        with patch("plugins._scheduler.helpers.job_loop.TaskScheduler.get") as mock_get:
...
def test_sleep_time_constant():
    from plugins._scheduler.helpers.job_loop import SLEEP_TIME
...
async def test_run_loop_attempts_pause_in_development():
    ...
    with patch("plugins._scheduler.helpers.job_loop.runtime.is_development", return_value=True):
        with patch("plugins._scheduler.helpers.job_loop.runtime.call_development_function", new_callable=AsyncMock) as mock_pause:
