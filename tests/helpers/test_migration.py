...
from helpers import migration


class TestMoveDir:
    def test_skips_when_src_not_exists(self):
        with patch("helpers.migration.files") as mf:
    ...
    def test_moves_when_src_exists_and_dst_not(self):
        with patch("helpers.migration.files") as mf:
            ...
            with patch("helpers.migration.PrintStyle"):
...
class TestMigrateUserData:
    def test_calls_move_operations(self):
        with patch("helpers.migration.files") as mf:
            ...
            with patch("helpers.migration.PrintStyle"):
                with patch("helpers.migration._migrate_memory"):
                    with patch("helpers.migration._merge_dir_contents"):
                        with patch("helpers.migration._cleanup_obsolete"):
