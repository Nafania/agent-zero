...
class TestGetSkillsBaseDir:
    def test_returns_path_with_usr_skills(self):
        with patch("helpers.skills.files") as mock_files:
            mock_files.get_abs_path.return_value = "/a0/usr/skills"
            from helpers.skills import get_skills_base_dir
...
class TestGetSkillRoots:
    ...
    def test_agent_roots_with_agent(self, mock_agent):
        with patch("helpers.skills.subagents") as mock_subagents, \
             patch("helpers.skills.os.path.isdir", return_value=False):
...
class TestIsHiddenPath:
    def test_dot_folder_hidden(self):
        from helpers.skills import _is_hidden_path
...
class TestCoerceList:
    def test_none_returns_empty(self):
        from helpers.skills import _coerce_list
...
class TestNormalizeName:
    def test_lowercase_and_hyphens(self):
        from helpers.skills import _normalize_name
...
class TestParseFrontmatter:
    def test_empty_returns_empty_dict(self):
        from helpers.skills import parse_frontmatter
...
class TestSkillFromMarkdown:
    def test_valid_skill_parsed(self, tmp_workdir):
        from helpers.skills import skill_from_markdown
...
class TestListSkills:
    def test_empty_when_no_roots(self):
        with patch("helpers.skills.get_skill_roots", return_value=[]):
    ...
    def test_finds_skills_from_roots(self, tmp_workdir):
        ...
        (skill_dir / "SKILL.md").write_text("---\nname: test-skill\ndescription: x\n---\n")
        with (
            patch("helpers.skills.get_skill_roots", return_value=[str(tmp_workdir)]),
            patch("helpers.skills.files") as mock_files,
        ):
            mock_files.normalize_a0_path.side_effect = lambda x: str(Path(x))
            from helpers.skills import list_skills
            skills = list_skills()
...
class TestLoadSkillForAgent:
    def test_returns_error_when_not_found(self):
        with patch("helpers.skills.find_skill", return_value=None):
            from helpers.skills import load_skill_for_agent
            result = load_skill_for_agent("nonexistent")
            assert "not found" in result

    def test_returns_formatted_skill_when_found(self):
        from helpers.skills import Skill, load_skill_for_agent
        skill = Skill(
            name="test-skill",
            description="A test",
            path=Path("/a/skill"),
        ...
        with (
            patch("helpers.skills.find_skill", return_value=skill),
            patch("helpers.skills.runtime") as mock_runtime,
            patch("helpers.skills._get_skill_files", return_value=""),
        ):
            mock_runtime.is_development.return_value = False
            from helpers.skills import load_skill_for_agent
...
class TestValidateSkill:
    def test_missing_name(self):
        from helpers.skills import validate_skill, Skill
...
class TestValidateSkillMd:
    def test_unreadable_file(self):
        from helpers.skills import validate_skill_md
