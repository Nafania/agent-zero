...
class TestGetSkillsBaseDir:
    def test_returns_path_with_usr_skills(self):
        with patch("plugins.skills.helpers.skills.files") as mock_files:
            mock_files.get_abs_path.return_value = "/a0/usr/skills"
            from plugins.skills.helpers.skills import get_skills_base_dir
...
class TestGetSkillRoots:
    def test_global_roots_without_agent(self):
        with patch("plugins.skills.helpers.skills.files") as mock_files:
            ...
            from plugins.skills.helpers.skills import get_skill_roots
    ...
    def test_agent_roots_with_agent(self, mock_agent):
        with patch("plugins.skills.helpers.skills.subagents") as mock_subagents, \
             patch("plugins.skills.helpers.skills.os.path.isdir", return_value=False):
...
class TestIsHiddenPath:
    def test_dot_folder_hidden(self):
        from plugins.skills.helpers.skills import _is_hidden_path
...
class TestCoerceList:
    def test_none_returns_empty(self):
        from plugins.skills.helpers.skills import _coerce_list
...
class TestNormalizeName:
    def test_lowercase_and_hyphens(self):
        from plugins.skills.helpers.skills import _normalize_name
...
class TestSplitFrontmatter:
    def test_valid_frontmatter(self):
        from plugins.skills.helpers.skills import split_frontmatter
        text = "---\nname: test\ndescription: desc\n---\n\nBody content"
...
class TestParseFrontmatter:
    def test_empty_returns_empty_dict(self):
        from plugins.skills.helpers.skills import parse_frontmatter
...
class TestSkillFromMarkdown:
    def test_valid_skill_parsed(self, tmp_workdir):
        from plugins.skills.helpers.skills import skill_from_markdown
...
class TestListSkills:
    def test_empty_when_no_roots(self):
        with patch("plugins.skills.helpers.skills.get_skill_roots", return_value=[]):
            from plugins.skills.helpers.skills import list_skills
            assert list_skills() == []
    ...
    def test_finds_skills_from_roots(self, tmp_workdir):
        ...
        (skill_dir / "SKILL.md").write_text("---\nname: test-skill\ndescription: x\n---\n")
        with (
            patch("plugins.skills.helpers.skills.get_skill_roots", return_value=[str(tmp_workdir)]),
            patch("plugins.skills.helpers.skills.files") as mock_files,
        ):
            mock_files.normalize_a0_path.side_effect = lambda x: str(Path(x))
            from plugins.skills.helpers.skills import list_skills
            skills = list_skills()
...
class TestFindSkill:
    def test_returns_none_for_empty_name(self):
        with patch("plugins.skills.helpers.skills.get_skill_roots", return_value=[]):
            from plugins.skills.helpers.skills import find_skill
...
class TestDeleteSkill:
    def test_raises_when_root_not_in_scope(self):
        with (
            patch("plugins.skills.helpers.skills.get_skill_roots", return_value=["/allowed/root"]),
            patch("plugins.skills.helpers.skills.files") as mock_files,
        ):
            mock_files.get_abs_path.side_effect = lambda *p: "/other/path/skill"
            mock_files.is_in_dir.return_value = False
            from plugins.skills.helpers.skills import delete_skill
            with pytest.raises(ValueError, match="not in current scope"):
                delete_skill("skill")

    def test_raises_when_directory_not_found(self):
        with (
            patch("plugins.skills.helpers.skills.get_skill_roots", return_value=["/root"]),
            patch("plugins.skills.helpers.skills.files") as mock_files,
            patch("plugins.skills.helpers.skills.runtime") as mock_runtime,
        ):
...
class TestLoadSkillForAgent:
    def test_returns_error_when_not_found(self):
        with patch("plugins.skills.helpers.skills.find_skill", return_value=None):
            from plugins.skills.helpers.skills import load_skill_for_agent
            result = load_skill_for_agent("nonexistent")
            assert "not found" in result

    def test_returns_formatted_skill_when_found(self):
        from plugins.skills.helpers.skills import Skill, load_skill_for_agent
        skill = Skill(
            name="test-skill",
            description="A test",
            path=Path("/a/skill"),
        ...
        with (
            patch("plugins.skills.helpers.skills.find_skill", return_value=skill),
            patch("plugins.skills.helpers.skills.runtime") as mock_runtime,
            patch("plugins.skills.helpers.skills._get_skill_files", return_value=""),
        ):
            mock_runtime.is_development.return_value = False
            from plugins.skills.helpers.skills import load_skill_for_agent
...
class TestSearchSkills:
    def test_empty_query_returns_empty(self):
        with patch("plugins.skills.helpers.skills.list_skills", return_value=[]):
            from plugins.skills.helpers.skills import search_skills
            assert search_skills("") == []
            assert search_skills("   ") == []

    def test_scores_by_name_match(self):
        from plugins.skills.helpers.skills import Skill, search_skills
        skills = [
            Skill("foo-bar", "desc", Path("/a"), Path("/a/SKILL.md")),
            Skill("other", "desc", Path("/b"), Path("/b/SKILL.md")),
        ]
        with patch("plugins.skills.helpers.skills.list_skills", return_value=skills):
...
class TestValidateSkill:
    def test_missing_name(self):
        from plugins.skills.helpers.skills import validate_skill, Skill
...
class TestValidateSkillMd:
    def test_unreadable_file(self):
        from plugins.skills.helpers.skills import validate_skill_md
