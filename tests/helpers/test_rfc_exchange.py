...
class TestGetRootPassword:
    async def test_get_root_password_dockerized_uses_dotenv(self):
        from helpers.rfc_exchange import get_root_password

        with patch("helpers.rfc_exchange.runtime.is_dockerized", return_value=True):
            with patch("helpers.rfc_exchange._get_root_password") as mock_get:
    ...
    async def test_get_root_password_non_docker_uses_crypto_exchange(self):
        ...
        with patch("helpers.rfc_exchange.runtime.is_dockerized", return_value=False):
            with patch("helpers.rfc_exchange.crypto") as mock_crypto:
                ...
                with patch("helpers.rfc_exchange.runtime.call_development_function", new_callable=AsyncMock) as mock_call:
...
class TestProvideRootPassword:
    def test_provide_root_password_returns_encrypted(self):
        from helpers.rfc_exchange import _provide_root_password

        with patch("helpers.rfc_exchange._get_root_password", return_value="secret"):
            with patch("helpers.rfc_exchange.crypto.encrypt_data") as mock_enc:
...
class TestGetRootPassword:
    def test_get_root_password_returns_dotenv_value(self):
        from helpers.rfc_exchange import _get_root_password

        with patch("helpers.rfc_exchange.dotenv.get_dotenv_value") as mock_get:
    ...
    def test_get_root_password_returns_empty_when_not_set(self):
        ...
        with patch("helpers.rfc_exchange.dotenv.get_dotenv_value", return_value=None):
            result = _get_root_password()
            assert result == ""
