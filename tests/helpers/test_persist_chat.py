...
def test_get_chat_folder_path() -> None:
    ...
    with patch("helpers.persist_chat.files") as mf:
...
def test_get_chat_msg_files_folder() -> None:
    from helpers.persist_chat import get_chat_msg_files_folder
...
def test_load_json_chats() -> None:
    from helpers.persist_chat import load_json_chats

    with patch("helpers.persist_chat._deserialize_context") as md:
        mock_ctx = MagicMock()
...
def test_remove_chat() -> None:
    ...
    with patch("helpers.persist_chat.files") as mf:
        with patch("helpers.persist_chat.get_chat_folder_path", return_value="/chats/ctx"):
            remove_chat("ctx")
    mf.delete_dir.assert_called_once_with("/chats/ctx")


def test_remove_msg_files() -> None:
    from helpers.persist_chat import remove_msg_files

    with patch("helpers.persist_chat.files") as mf:
        with patch("helpers.persist_chat.get_chat_msg_files_folder", return_value="/chats/ctx/messages"):
...
def test_safe_json_serialize_skips_non_serializable() -> None:
    from helpers.persist_chat import _safe_json_serialize
