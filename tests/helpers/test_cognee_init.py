...
class TestMapProvider:
    def test_known_providers(self):
        from plugins.memory.helpers.cognee_init import _map_provider
...
class TestGetApiKey:
    def test_env_var_takes_priority(self):
        from plugins.memory.helpers.cognee_init import _get_api_key
        with patch("plugins.memory.helpers.cognee_init.dotenv") as mock_dotenv:
    ...
    def test_settings_fallback(self):
        from plugins.memory.helpers.cognee_init import _get_api_key
        with patch("plugins.memory.helpers.cognee_init.dotenv") as mock_dotenv, \
             patch("plugins.memory.helpers.cognee_init.get_settings") as mock_settings:
            mock_dotenv.load_dotenv.return_value = None
            mock_dotenv.get_dotenv_value.return_value = None
...
class TestConfigureCognee:
    ...
    def test_idempotent_only_runs_once(self):
        ...
        with patch("plugins.memory.helpers.cognee_init.dotenv") as mock_dotenv, \
             patch("plugins.memory.helpers.cognee_init.get_settings", return_value=self._mock_settings()), \
             patch.dict("sys.modules", {"cognee": mock_cognee}), \
             patch("plugins.memory.helpers.cognee_init.files"):
            mock_dotenv.load_dotenv.return_value = None
            mock_dotenv.get_dotenv_value.return_value = None
            ci.configure_cognee()
...
class TestConfigureCogneeAdditional:
    ...
    def test_configure_cognee_sets_embedding_for_non_fastembed(self, _clean_env):
        ...
        with patch("plugins.memory.helpers.cognee_init.dotenv") as mock_dotenv, \
             patch("plugins.memory.helpers.cognee_init.get_settings", return_value=settings), \
             patch.dict("sys.modules", {"cognee": mock_cognee}), \
             patch("plugins.memory.helpers.cognee_init.files") as mock_files:
            mock_dotenv.load_dotenv.return_value = None
            mock_dotenv.get_dotenv_value.return_value = None
...
class TestInitCognee:
    ...
    async def test_init_cognee_calls_configure_and_create_tables(self):
        ...
        with patch("plugins.memory.helpers.cognee_init.configure_cognee") as mock_configure, \
