...
class TestGetDefaultValue:
    def test_returns_default_when_env_not_set(self):
        with patch("helpers.settings.dotenv") as mock_dotenv:
            mock_dotenv.get_dotenv_value.return_value = None
    ...
    def test_invalid_value_returns_default_and_warns(self):
        with patch("helpers.settings.dotenv") as mock_dotenv:
            ...
            with patch("helpers.settings.PrintStyle") as mock_ps:
    ...
    def test_invalid_json_returns_default(self):
        with patch("helpers.settings.dotenv") as mock_dotenv:
            ...
            with patch("helpers.settings.PrintStyle"):
                result = settings_module.get_default_value("foo", {})
        assert result == {}
...
class TestGetSettings:
    def test_returns_defaults_when_no_file(self):
        with patch("helpers.settings._read_settings_file", return_value=None):
            with patch("helpers.settings.get_default_settings") as mock_def:
                mock_def.return_value = _minimal_settings()
            with patch("helpers.settings._load_sensitive_settings"):
    ...
    def test_returns_file_settings_when_file_exists(self):
        ...
        with patch("helpers.settings._read_settings_file", return_value=file_settings):
            with patch("helpers.settings.get_default_settings", return_value=_minimal_settings()):
                with patch("helpers.settings._load_sensitive_settings"):
                    result = settings_module.get_settings()
    ...
    def test_returns_cached_settings_on_second_call(self):
        ...
        with (
            patch("helpers.settings._read_settings_file", return_value=None),
            patch("helpers.settings.get_default_settings", return_value=minimal),
            patch("helpers.settings._load_sensitive_settings"),
        ):
...
class TestSetSettings:
    def test_writes_file_and_reloads(self):
        s = _minimal_settings()
        with patch("helpers.settings._write_settings_file") as mock_write:
            with patch("helpers.settings._apply_settings"):
    ...
    def test_apply_true_calls_apply_settings(self):
        s = _minimal_settings()
        with patch("helpers.settings._write_settings_file"):
            with patch("helpers.settings._apply_settings") as mock_apply:
...
class TestSetSettingsDelta:
    def test_merges_delta_into_current(self):
        with patch("helpers.settings.get_settings") as mock_get:
            mock_get.return_value = _minimal_settings()
            with patch("helpers.settings.set_settings") as mock_set:
