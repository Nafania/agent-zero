...
def _reset_singleton():
    ...
    import helpers.fasta2a_server as fas
...
class TestIsAvailableAndGetProxy:
    def test_get_proxy_returns_singleton(self):
        with patch("helpers.fasta2a_server.settings") as mock_settings_mod:
            mock_settings_mod.get_settings.return_value = {"mcp_server_token": "", "a2a_server_enabled": True}
            with patch("helpers.fasta2a_server.FASTA2A_AVAILABLE", True):
                with patch("helpers.fasta2a_server.FastA2A"):
                    with patch("helpers.fasta2a_server.InMemoryBroker"):
                        with patch("helpers.fasta2a_server.InMemoryStorage"):
                            with patch("helpers.fasta2a_server.PrintStyle"):
                                from helpers.fasta2a_server import get_proxy, DynamicA2AProxy
    ...
    def test_is_available_false_when_fasta2a_not_installed(self):
        with patch("helpers.fasta2a_server.FASTA2A_AVAILABLE", False):
            with patch("helpers.fasta2a_server.settings") as m:
...
class TestAgentZeroWorkerConvertMessage:
    def test_convert_message_text_only(self):
        with patch("helpers.fasta2a_server.FASTA2A_AVAILABLE", True):
            with patch("helpers.fasta2a_server.Worker"):
                with patch("helpers.fasta2a_server.InMemoryBroker"):
                    with patch("helpers.fasta2a_server.InMemoryStorage"):
                        from helpers.fasta2a_server import AgentZeroWorker
            ...
            with patch("helpers.fasta2a_server.Worker"):
                with patch("helpers.fasta2a_server.InMemoryBroker"):
                    with patch("helpers.fasta2a_server.InMemoryStorage"):
                        ...
                        with patch("helpers.fasta2a_server.initialize_agent", return_value=mock_cfg):
                            with patch("helpers.fasta2a_server.AgentContext", return_value=mock_context):
                                with patch("helpers.fasta2a_server.AgentContextType"):
                                    with patch("helpers.fasta2a_server.projects"):
                                        with patch("helpers.fasta2a_server.remove_chat"):
                                            with patch("helpers.fasta2a_server.AgentContext.remove"):
                                                with patch("helpers.fasta2a_server.PrintStyle"):
            ...
            with patch("helpers.fasta2a_server.Worker"):
                with patch("helpers.fasta2a_server.InMemoryBroker"):
                    with patch("helpers.fasta2a_server.InMemoryStorage"):
                        ...
                        with patch("helpers.fasta2a_server.initialize_agent", side_effect=RuntimeError("Boom")):
                            with patch("helpers.fasta2a_server.AgentContext"):
                                with patch("helpers.fasta2a_server.projects"):
                                    with patch("helpers.fasta2a_server.remove_chat"):
                                        with patch("helpers.fasta2a_server.AgentContext.remove"):
                                            with patch("helpers.fasta2a_server.PrintStyle"):
                ...
                with patch("helpers.fasta2a_server.FastA2A"):
                    with patch("helpers.fasta2a_server.InMemoryBroker"):
                        with patch("helpers.fasta2a_server.InMemoryStorage"):
                            ...
                            with patch("helpers.settings.get_settings", return_value=cfg):
                                await proxy(scope, receive, send)
