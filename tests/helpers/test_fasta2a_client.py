...
def test_is_client_available():
    ...
    from plugins._a2a.helpers.fasta2a_client import is_client_available, FASTA2A_CLIENT_AVAILABLE
    assert is_client_available() is FASTA2A_CLIENT_AVAILABLE


@pytest.mark.skipif(
    not getattr(__import__("plugins._a2a.helpers.fasta2a_client", fromlist=["FASTA2A_CLIENT_AVAILABLE"]), "FASTA2A_CLIENT_AVAILABLE"),
...
class TestAgentConnectionWhenAvailable:
    ...
    def test_init_adds_http_scheme_when_missing(self):
        from plugins._a2a.helpers.fasta2a_client import AgentConnection
        with patch("plugins._a2a.helpers.fasta2a_client.httpx") as mock_httpx, \
             patch("plugins._a2a.helpers.fasta2a_client.A2AClient"):
    ...
    async def test_get_agent_card_success(self, mock_http_client, mock_a2a_client):
        ...
        with patch("plugins._a2a.helpers.fasta2a_client.httpx") as mock_httpx, \
             patch("plugins._a2a.helpers.fasta2a_client.A2AClient") as mock_a2a_cls:
...
class TestConnectToAgent:
    ...
    async def test_connect_to_agent_returns_connection(self):
        from plugins._a2a.helpers.fasta2a_client import connect_to_agent, AgentConnection

        mock_resp = MagicMock()
        mock_resp.json.return_value = {"name": "TestAgent"}
        mock_resp.raise_for_status = MagicMock()

        with patch("plugins._a2a.helpers.fasta2a_client.AgentConnection") as mock_cls, \
             patch("plugins._a2a.helpers.fasta2a_client.httpx") as mock_httpx, \
             patch("plugins._a2a.helpers.fasta2a_client.A2AClient"):
            mock_conn = MagicMock()
...
class TestAgentConnectionWhenUnavailable:
    ...
    def test_init_raises_when_unavailable(self):
        import plugins._a2a.helpers.fasta2a_client as fasta2a_client
