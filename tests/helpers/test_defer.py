...
class TestEventLoopThread:
    def test_event_loop_thread_singleton_per_name(self):
        from helpers.defer import EventLoopThread, THREAD_BACKGROUND

        try:
            with patch("helpers.defer.EventLoopThread._start"):
                with patch("helpers.defer.EventLoopThread._lock"):
...
class TestDeferredTask:
    ...
    def test_start_task_raises_before_start_for_result_sync(self):
        ...
        with patch("helpers.defer.EventLoopThread"):
            task = DeferredTask()
...
class TestChildTask:
    def test_child_task_dataclass(self):
        from helpers.defer import ChildTask, DeferredTask
