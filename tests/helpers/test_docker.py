...
class TestDockerContainerManager:
    ...
    def test_init_stores_params(self):
        ...
        with patch("plugins._code_execution.helpers.docker.docker") as mock_docker:
            mock_client = MagicMock()
            mock_docker.from_env.return_value = mock_client

            from plugins._code_execution.helpers.docker import DockerContainerManager
    ...
    def test_init_docker_retries_on_connection_refused(self):
        ...
        with patch("plugins._code_execution.helpers.docker.docker") as mock_docker:
            with patch("plugins._code_execution.helpers.docker.format_error") as mock_fmt:
                mock_fmt.return_value = "ConnectionRefusedError(61, ...)"
                mock_docker.from_env.side_effect = [
                    ConnectionRefusedError(),
                    MagicMock(),
                ]
                with patch("plugins._code_execution.helpers.docker.time.sleep"):
                    from plugins._code_execution.helpers.docker import DockerContainerManager
    ...
    def test_cleanup_container_stops_and_removes(self):
        ...
        with patch("plugins._code_execution.helpers.docker.docker"):
            from plugins._code_execution.helpers.docker import DockerContainerManager
