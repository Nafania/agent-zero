...
def patch_files_and_skills(tmp_path):
    ...
    with patch("plugins.skills.helpers.skills_import.files") as mock_files:
        mock_files.get_abs_path.side_effect = lambda *parts: str(
            tmp_path.joinpath(*[str(p) for p in parts])
        )
        with patch(
            "plugins.skills.helpers.skills_import.discover_skill_md_files"
        ) as mock_discover:
...
class TestIsWithin:
    def test_child_inside_parent(self, tmp_path):
        from plugins.skills.helpers.skills_import import _is_within
...
class TestDeriveNamespace:
    def test_zip_stem(self):
        from plugins.skills.helpers.skills_import import _derive_namespace
...
class TestResolveConflict:
    def test_nonexistent_returns_copy_true(self):
        from plugins.skills.helpers.skills_import import _resolve_conflict
...
class TestBuildImportPlan:
    def test_builds_plan_from_skill_dirs(self, tmp_skills_source, patch_files_and_skills):
        from plugins.skills.helpers.skills_import import build_import_plan
...
class TestResolveSkillsDestinationRoot:
    def test_project_and_profile(self):
        from plugins.skills.helpers.skills_import import resolve_skills_destination_root

        with patch(
            "plugins.skills.helpers.skills_import.get_project_agent_profile_skills_folder"
        ) as mock_get:
    ...
    def test_project_only(self):
        ...
        with patch(
            "plugins.skills.helpers.skills_import.get_project_skills_folder"
        ) as mock_get:
    ...
    def test_profile_only(self):
        ...
        with patch(
            "plugins.skills.helpers.skills_import.get_agent_profile_skills_folder"
        ) as mock_get:
...
class TestImportSkills:
    def test_raises_when_source_not_found(self, patch_files_and_skills):
        from plugins.skills.helpers.skills_import import import_skills
