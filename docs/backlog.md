...
| A2 | **Path restructuring** — `python/helpers/` → `helpers/`, `python/tools/` → `tools/`, `python/extensions/` → `extensions/python/`. All imports change. | Upstream commit `d02dda3667` "BIG PYTHON REFACTOR". | — | L | Affects all 300+ Python files. Cherry-picking any upstream commit after this requires import translation. |
...
| A5 | **WebSocket handlers → extensions** — `python/websocket_handlers/` removed, logic absorbed into extension hooks. Dynamic WS endpoint registration. | Upstream v1.1. | A1, A3 | M | Coupled to plugin system and @extensible. |
...
| 6 | **CSRF fails on Chromium over HTTPS** — WebSocket CSRF validation broken on Chrome when using HTTPS. Fix validates origin correctly. | Upstream commit `07f94ef4b5`. File: `python/helpers/security.py` (upstream: `helpers/security.py`). | UPSTREAM | XS |
| 7 | **CSRF cookie missing Secure flag** — cookie sent without `Secure` flag on HTTPS connections, allowing MitM interception. | Upstream commit `9e3bbb759f`. File: `python/helpers/security.py`. | UPSTREAM | XS |
| 8 | **Open redirect on login** — login redirect URL not validated, allows redirect to external sites. | Upstream commit `da24eb04e0`. File: `python/helpers/login.py`. | UPSTREAM | XS |
| 9 | **Context compression deadlock** — agent freezes when context window compression is triggered under certain conditions. | Upstream commit `1db8e3f095`. File: `python/helpers/history.py` or `python/helpers/tokens.py`. | UPSTREAM | S |
...
| 11 | **Skills loading KeyError** — `loaded_skills` accessed with direct key lookup before it's populated; crashes on first skill use. Fix: use `.get()`. | Upstream commits `e8abe7101f`, `f821069734`. File: `python/tools/skills_tool.py`. | UPSTREAM | XS |
| 12 | **Settings delta clears authentication** — saving any setting resets auth state, logging the user out. | Upstream commit `343b141178`. File: `python/api/settings_set.py`. | UPSTREAM | XS |
| 13 | **Remove version check** — `update_check.py` calls `api.agent-zero.ai` to check for original Agent Zero updates. Irrelevant for fork, potentially confusing (suggests updating to original). | Files: `python/helpers/update_check.py`, `python/extensions/user_message_ui/_10_update_check.py`, `python/helpers/settings.py` (`update_check_enabled`), tests. | BUILD (remove) | S |
...
| 22 | **Parallel MCP initialization** — MCP servers init in parallel instead of sequentially. Cuts startup time with multiple MCP servers configured. | Upstream commit `6734b9dc0f`. File: `python/helpers/mcp_handler.py`. | UPSTREAM | S |
...
| 34 | **Vision tool empty content crash** — `_convert_messages` crashes on empty content from vision tool. Fix: skip empty content, add text to vision results. | Upstream commits `a7df75a9dd`, `f9743cb736`. File: `python/helpers/call_llm.py`, `python/tools/vision_load.py`. | UPSTREAM | XS |
| 35 | **Cross-device file move fails** — `os.rename` fails across Docker volume mount points. Fallback to `shutil.move`. Affects chat/project operations in Docker. | Upstream commit `0e5e9851d4`. File: `python/helpers/files.py`. | UPSTREAM | XS |
| 36 | **TTY queue event loop mismatch** — event loop mismatch in TTY queue causes code execution failures in async contexts. | Upstream commit `0582c51998`. File: `python/helpers/tty_session.py`. | UPSTREAM | XS |
| 37 | **Utility calls non-stream** — switch utility model calls (title generation, memory ops) from streaming to non-stream mode. Less overhead, faster responses for short outputs. | Upstream commit `d05224b895`. File: `python/helpers/call_llm.py`. | UPSTREAM | XS |
| 38 | **Secret masking in code execution** — mask `§§secret()` values in terminal output so agents don't leak credentials in conversation history. Also increases code output truncation limit. | Upstream commit `6181ac9c20`. File: `python/tools/code_execution_tool.py`. | UPSTREAM | S |
...
| 55 | **File browser dropdown fix** — file browser dropdown broken; context ID not validated before fetching chat files path. | Upstream commits `1880eb09d4`, `ec4de76561`. Files: `python/helpers/file_browser.py`, `python/api/chat_files_path_get.py`. | UPSTREAM | XS |
| 56 | **Copy file fallback** — no fallback when file copy encounters OS errors (permissions, cross-device). | Upstream commit `4fa70fcdfb`. File: `python/helpers/files.py`. | UPSTREAM | XS |
| 57 | **SearXNG settings fix** — SearXNG search engine configuration not applied correctly. | Upstream commit `1f7e3b5a28`. File: `python/helpers/searxng.py`. | UPSTREAM | XS |
| 58 | **Scheduler API fix** — scheduler API calls broken, tasks not triggering correctly. | Upstream commit `68e65ae873`. File: `python/api/scheduler_*.py`. | UPSTREAM | XS |
