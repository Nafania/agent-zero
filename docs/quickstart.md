...
Using Docker Desktop GUI, search for `ghcr.io/nafania/agent-zero` and click Pull, or use the terminal:
...
**Using Docker Desktop:** Go to Images tab, click Run next to `ghcr.io/nafania/agent-zero`, open Optional settings, map a host port to container port `80` (use `0` for automatic assignment), then click Run.

**Using Terminal:**

```bash
docker run -p 0:80 ghcr.io/nafania/agent-zero
```
